...
const sectionConfig = {
  marketingUseCases: { title: 'Marketing Use Cases', fields: ['title', 'description', 'points'] },
  workflowStages: { title: 'Workflow Stages', fields: ['phase', 'title', 'text'] },
  capabilityTags: { title: 'Capability Tags', fields: ['text'] },
  resultCards: { title: 'Result Cards', fields: ['value', 'label'] },
...
export default function GenerativeAIMarketingManagement() {
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/generative-ai-marketing-page')
      setPageData(data)
      setPageForm({
        ...initialPageForm,
      ...
      setError(requestError.message || 'Failed to load generative ai marketing page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/generative-ai-marketing-page', {
        method: 'PUT',
        body: JSON.stringify(pageForm),
      })
      setMessage(result.message || 'Generative AI marketing page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      const path = editingId
        ? `/api/generative-ai-marketing-page/${section}/${editingId}`
        : `/api/generative-ai-marketing-page/${section}`
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/generative-ai-marketing-page/${section}/${itemId}`, { method: 'DELETE' })
  ...
  return (
    <DashboardShell
      title="Generative AI Marketing Management"
      subtitle="Ei dashboard theke GenerativeAIMarketing page er shokol content (hero, intro, use-cases, workflow, capabilities, result cards, faq, CTA) CRUD korte parbe."
    >
