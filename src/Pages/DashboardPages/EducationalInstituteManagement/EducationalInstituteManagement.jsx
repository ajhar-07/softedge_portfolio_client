...
const API_PAGE = '/api/educational-institute-management-page'
...
const initialForms = {
  ...
  socials: { label: '' },
  cards: { title: '', description: '', image: '', icon: 'book', variant: 'overlay' },
  featureBullets: { text: '' },
...
export default function EducationalInstituteManagement() {
  ...
  const loadPageData = async () => {
    ...
    try {
      ...
      setPageForm({
        ...
        stripLabel: data.stripLabel || 'EIMS',
      ...
      setError(requestError.message || 'Failed to load educational institute management page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    try {
      ...
      setMessage(result.message || 'Educational institute management page updated successfully')
      await loadPageData()
    } catch (requestError) {
        ...
        {section === 'serviceLinks' ? (
          <>
            ...
            <input
              ...
              value={form.to}
              onChange={(event) => handleSectionFormChange(section, event)}
              placeholder="/educational-institute-management"
              className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40"
              required
            />
          </>
  ...
  return (
    <DashboardShell
      ...
      subtitle={
        <>
          Public page:{' '}
          <Link to="/educational-institute-management" className="text-[#00d2ff] underline hover:text-white">
