...
const sectionConfig = {
  serviceModules: { title: 'Service Modules', fields: ['title', 'description', 'points'] },
  governancePillars: { title: 'Governance Pillars', fields: ['text'] },
  implementationPlan: { title: 'Implementation Plan', fields: ['phase', 'title', 'text'] },
  videoItems: { title: 'Video Items', fields: ['title', 'embedUrl'] },
  galleryItems: { title: 'Gallery Items', fields: ['image', 'alt'] },
}
...
export default function ServicePointManagement() {
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/service-point-page')
      setPageData(data)
      setPageForm({
        ...initialPageForm,
      ...
      setError(requestError.message || 'Failed to load service point page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/service-point-page', {
        method: 'PUT',
        body: JSON.stringify(pageForm),
      })
      setMessage(result.message || 'Service point page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      const path = editingId ? `/api/service-point-page/${section}/${editingId}` : `/api/service-point-page/${section}`
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/service-point-page/${section}/${itemId}`, { method: 'DELETE' })
  ...
  return (
    <DashboardShell
      ...
      subtitle="Ei dashboard theke ServicePoint page er shokol content (hero, modules, governance, roadmap, videos, gallery, CTA) CRUD korte parbe."
    >
              ...
              <form className="mt-3 space-y-2.5 rounded-xl border border-white/10 bg-white/[0.02] p-3" onSubmit={(event) => handleSectionSubmit(section, event)}>
                {sectionConfig[section].fields.map((field) => (
                  <div key={`${section}-${field}`}>
                    ...
                    {field === 'description' || field === 'points' || field === 'text' || field === 'alt' ? (
                      <textarea
                        name={field}
              ...
              <ul className="mt-3 grid gap-2">
                {(pageData?.[section] || []).map((item) => (
                  <li key={item._id} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
                    <p className="text-sm text-white">{item.title || item.phase || item.text || item.embedUrl || item.image}</p>
                    {(item.description || item.alt) ? <p className="text-xs text-white/70">{item.description || item.alt}</p> : null}
