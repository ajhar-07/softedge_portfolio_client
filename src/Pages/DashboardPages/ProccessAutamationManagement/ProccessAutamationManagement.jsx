...
const initialForms = {
  ...
  socials: { label: '' },
  cards: { title: '', description: '', image: '', icon: 'code', variant: 'overlay' },
...
export default function ProccessAutamationManagement() {
  const [pageForm, setPageForm] = useState(initialPageForm)
  const [sectionForms, setSectionForms] = useState(initialForms)
  const [editing, setEditing] = useState({ serviceLinks: '', socials: '', cards: '', faqs: '' })
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/process-automation-page')
      setPageData(data)
      setPageForm({
        heroImage: data.heroImage || '',
        heroStripImage: data.heroStripImage || '',
        heroTitle: data.heroTitle || 'Process Automation',
        sectionTitle: data.sectionTitle || 'Process Automation',
        sectionDescription: data.sectionDescription || '',
        sectionDescriptionBottom: data.sectionDescriptionBottom || '',
        finalDescription: data.finalDescription || '',
        stripLabel: data.stripLabel || 'Automation',
      ...
      setError(requestError.message || 'Failed to load process automation page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/process-automation-page', {
        method: 'PUT',
        body: JSON.stringify(pageForm),
      })
      setMessage(result.message || 'Process automation page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      const path = editingId
        ? `/api/process-automation-page/${section}/${editingId}`
        : `/api/process-automation-page/${section}`
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/process-automation-page/${section}/${itemId}`, {
        method: 'DELETE',
      })
  ...
  return (
    <DashboardShell
      title="Proccess Autamation Management"
      subtitle="Ei dashboard page theke process automation page er shob content create, read, update, delete korte parben."
    >
      <div className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="rounded-[28px] border border-white/10 bg-[#000b1e]/42 p-5 shadow-[0_12px_35px_-15px_rgba(0,0,0,0.52)] sm:p-6">
          ...
          <h2 className="mt-3 text-2xl font-black text-white">Live process automation content</h2>
          ...
          <div className="mt-6 space-y-5">
            {loading ? (
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-8 text-center text-sm text-white/68">
                Loading process automation data...
              </div>
            ) : pageData ? (
              <>
                {Object.keys(sectionLabels).map((section) => (
