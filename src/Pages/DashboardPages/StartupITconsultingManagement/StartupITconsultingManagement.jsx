...
const sectionConfig = {
  consultingPillars: { title: 'Consulting Pillars', fields: ['title', 'description'] },
  transformationStages: { title: 'Transformation Stages', fields: ['step', 'title', 'text'] },
  outcomes: { title: 'Expected Outcomes', fields: ['text'] },
}
...
export default function StartupITconsultingManagement() {
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/startup-it-consulting-page')
      setPageData(data)
      setPageForm({
        ...initialPageForm,
      ...
      setError(requestError.message || 'Failed to load startup IT consulting page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/startup-it-consulting-page', { method: 'PUT', body: JSON.stringify(pageForm) })
      setMessage(result.message || 'Startup IT consulting page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      const path = editingId
        ? `/api/startup-it-consulting-page/${section}/${editingId}`
        : `/api/startup-it-consulting-page/${section}`
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/startup-it-consulting-page/${section}/${itemId}`, { method: 'DELETE' })
  ...
  return (
    <DashboardShell
      title="Startup IT Consulting Management"
      subtitle="Ei dashboard theke StartupITconsulting page er shokol content (hero, pillars, stages, outcomes, CTA) CRUD korte parbe."
    >
              ...
              <ul className="mt-3 grid gap-2">
                {(pageData?.[section] || []).map((item) => (
                  <li key={item._id} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
                    <p className="text-sm text-white">{item.title || item.step || item.text}</p>
