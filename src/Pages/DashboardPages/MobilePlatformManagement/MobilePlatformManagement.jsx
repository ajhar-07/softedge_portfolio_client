...
export default function MobilePlatformManagement() {
  ...
  const loadPageData = async () => {
    ...
    try {
      const data = await apiRequest('/api/mobile-platform-page')
      setPageData(data)
      setPageForm({
        heroImage: data.heroImage || '',
        topImageLeft: data.topImageLeft || '',
        topImageRight: data.topImageRight || '',
        heroTitle: data.heroTitle || 'Mobile Platforms',
        sectionTitle: data.sectionTitle || 'Mobile Platforms',
      ...
      setError(requestError.message || 'Failed to load mobile platform page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSectionFormChange = (section, event) => {
    ...
    setSectionForms((current) => ({
      ...
      [section]: {
        ...current[section],
        [name]: section === 'skills' && name === 'value' ? Number(value) : value,
      },
    }))
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    try {
      const result = await apiRequest('/api/mobile-platform-page', {
        method: 'PUT',
        body: JSON.stringify(pageForm),
      })
      setMessage(result.message || 'Mobile platform page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      const path = editingId
        ? `/api/mobile-platform-page/${section}/${editingId}`
        : `/api/mobile-platform-page/${section}`
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    try {
      const result = await apiRequest(`/api/mobile-platform-page/${section}/${itemId}`, {
        method: 'DELETE',
      })
        ...
        {section === 'team' ? (
          <>
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-white/82">Member Image Upload</span>
              <input
                type="file"
                accept="image/*"
              ...
              <p className="mt-2 text-xs text-white/50">
                {uploadingField === 'team-image' ? 'Uploading member image...' : 'Choose an image file to upload'}
              </p>
  ...
  const renderItemTitle = (section, item) => {
    if (section === 'serviceLinks' || section === 'socials' || section === 'skills') return item.label
  ...
  const renderItemDescription = (section, item) => {
    ...
    if (section === 'team') return `${item.role || ''}${item.image ? ` | ${item.image}` : ''}`
    return `${item.value}%`
  }

  return (
    <DashboardShell
      title="Mobile Platform Management"
      subtitle="Ei dashboard page theke mobile platform page er shob content create, read, update, delete korte parben."
    >
      <div className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="rounded-[28px] border border-white/10 bg-[#000b1e]/42 p-5 shadow-[0_12px_35px_-15px_rgba(0,0,0,0.52)] sm:p-6">
            ...
            <form className="mt-6 space-y-4" onSubmit={handlePageSubmit}>
              ...
              <div className="space-y-4">
                ...
                <label className="block">
                  <span className="mb-2 block text-sm font-medium text-white/82">Top Left Image Upload</span>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(event) => handleFileUpload(event, 'topImageLeft')}
                    className="block w-full text-sm text-white/75 file:mr-4 file:rounded-xl file:border-0 file:bg-[#00d2ff] file:px-4 file:py-2 file:text-sm file:font-bold file:text-[#000b1e]"
                  />
                  <p className="mt-2 text-xs text-white/50">
                    {uploadingField === 'topImageLeft' ? 'Uploading left image...' : 'Choose an image file to upload'}
                  </p>
                  {pageForm.topImageLeft ? (
                    <p className="mt-2 break-all text-xs text-white/45">{pageForm.topImageLeft}</p>
                  ) : null}
                </label>

                <label className="block">
                  <span className="mb-2 block text-sm font-medium text-white/82">Top Right Image Upload</span>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(event) => handleFileUpload(event, 'topImageRight')}
                    className="block w-full text-sm text-white/75 file:mr-4 file:rounded-xl file:border-0 file:bg-[#00d2ff] file:px-4 file:py-2 file:text-sm file:font-bold file:text-[#000b1e]"
                  />
                  <p className="mt-2 text-xs text-white/50">
                    {uploadingField === 'topImageRight' ? 'Uploading right image...' : 'Choose an image file to upload'}
                  </p>
                  {pageForm.topImageRight ? (
                    <p className="mt-2 break-all text-xs text-white/45">{pageForm.topImageRight}</p>
                  ) : null}
                </label>
              </div>
        ...
        <div className="rounded-[28px] border border-white/10 bg-[#000b1e]/42 p-5 shadow-[0_12px_35px_-15px_rgba(0,0,0,0.52)] sm:p-6">
          ...
          <h2 className="mt-3 text-2xl font-black text-white">Live mobile platform content</h2>
