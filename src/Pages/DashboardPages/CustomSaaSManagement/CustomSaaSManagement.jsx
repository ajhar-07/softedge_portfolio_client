...
const sectionConfig = {
  deliveryTracks: { title: 'Delivery Tracks', fields: ['title', 'description', 'points'] },
  buildFlow: { title: 'Working Process Steps', fields: ['stage', 'title', 'text'] },
  architectureHighlights: { title: 'Architecture Highlights', fields: ['text'] },
  engagementModels: { title: 'Engagement Models', fields: ['title', 'timeline', 'fit'] },
  projects: { title: 'Project Showcase Items', fields: ['name', 'category', 'summary', 'image', 'liveUrl', 'githubUrl'] },
  projectDemoVideos: { title: 'Project Demo Videos', fields: ['title', 'embedUrl'] },
  saasFaqs: { title: 'FAQs', fields: ['q', 'a'] },
}
...
export default function CustomSaaSManagement() {
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/custom-saas-page')
      setPageData(data)
      setPageForm({
        ...initialPageForm,
      ...
      setError(requestError.message || 'Failed to load custom saas page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/custom-saas-page', { method: 'PUT', body: JSON.stringify(pageForm) })
      setMessage(result.message || 'Custom saas page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      const path = editingId ? `/api/custom-saas-page/${section}/${editingId}` : `/api/custom-saas-page/${section}`
  ...
  const handleEditItem = (section, item) => {
    ...
    const nextValues = Object.fromEntries(config.fields.map((field) => [field, field === 'points' ? toMultiline(item[field]) : item[field] || '']))
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/custom-saas-page/${section}/${itemId}`, { method: 'DELETE' })
  ...
  return (
    <DashboardShell
      title="Custom SaaS Management"
      subtitle="Ei dashboard theke CustomSaaS page er shokol content (hero, tracks, working process, architecture, showcase, demos, FAQ) CRUD korte parbe."
    >
              ...
              <form className="mt-3 space-y-2.5 rounded-xl border border-white/10 bg-white/[0.02] p-3" onSubmit={(event) => handleSectionSubmit(section, event)}>
                {sectionConfig[section].fields.map((field) => (
                  <div key={`${section}-${field}`}>
                    ...
                    {(field === 'description' || field === 'points' || field === 'text' || field === 'summary' || field === 'fit' || field === 'a') ? (
                      <textarea
                        name={field}
              ...
              <ul className="mt-3 grid gap-2">
                {(pageData?.[section] || []).map((item) => (
                  <li key={item._id} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
                    <p className="text-sm text-white">{item.title || item.name || item.stage || item.text || item.q}</p>
                    {(item.description || item.summary || item.fit || item.a) ? (
                      <p className="text-xs text-white/70">{item.description || item.summary || item.fit || item.a}</p>
                    ) : null}
