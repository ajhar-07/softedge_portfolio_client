...
const sectionConfig = {
  solutionBlocks: { title: 'Solution Blocks', fields: ['title', 'description', 'bullets'] },
  capabilities: { title: 'Capabilities', fields: ['text'] },
  aiWorkflow: { title: 'AI Workflow', fields: ['phase', 'title', 'text'] },
  kpiCards: { title: 'KPI Cards', fields: ['value', 'label'] },
}
...
export default function ConversationalAIManagement() {
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/conversational-ai-page')
      setPageData(data)
      setPageForm({
        ...initialPageForm,
      ...
      setError(requestError.message || 'Failed to load conversational ai page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/conversational-ai-page', {
        method: 'PUT',
        body: JSON.stringify(pageForm),
      })
      setMessage(result.message || 'Conversational AI page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      if ('bullets' in payload) payload.bullets = fromMultiline(payload.bullets)
      const editingId = editing[section]
      const path = editingId
        ? `/api/conversational-ai-page/${section}/${editingId}`
        : `/api/conversational-ai-page/${section}`
  ...
  const handleEditItem = (section, item) => {
    ...
    const nextValues = Object.fromEntries(
      config.fields.map((field) => [field, field === 'bullets' ? toMultiline(item[field]) : item[field] || '']),
    )
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/conversational-ai-page/${section}/${itemId}`, { method: 'DELETE' })
  ...
  return (
    <DashboardShell
      ...
      subtitle="Ei dashboard theke ConversationalAI page er hero, content blocks, workflow, KPI cards, capabilities and CTA shokol content CRUD korte parbe."
    >
              ...
              <form className="mt-3 space-y-2.5 rounded-xl border border-white/10 bg-white/[0.02] p-3" onSubmit={(event) => handleSectionSubmit(section, event)}>
                {sectionConfig[section].fields.map((field) => (
                  <div key={`${section}-${field}`}>
                    ...
                    {field === 'description' || field === 'bullets' || field === 'text' || field === 'details' || field === 'a' ? (
                      <textarea
                        name={field}
                        rows={field === 'bullets' ? 3 : 2}
                        placeholder={field === 'bullets' ? 'One bullet per line' : ''}
              ...
              <ul className="mt-3 grid gap-2">
                {(pageData?.[section] || []).map((item) => (
                  <li key={item._id} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
                    <p className="text-sm text-white">{item.title || item.phase || item.value || item.text}</p>
                    {(item.description || item.label || item.a) ? (
                      <p className="text-xs text-white/70">{item.description || item.label || item.a}</p>
                    ) : null}
                    {item.bullets?.length ? <p className="mt-1 text-xs text-white/60">{item.bullets.join(', ')}</p> : null}
