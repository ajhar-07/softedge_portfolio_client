...
const sectionConfig = {
  supportPillars: { title: 'Support Pillars', fields: ['title', 'description', 'points'] },
  amcCoverage: { title: 'AMC Coverage', fields: ['text'] },
  responseMatrix: { title: 'Response Matrix', fields: ['severity', 'time', 'window', 'example'] },
  supportVideos: { title: 'Support Videos', fields: ['title', 'url'] },
  amcPlans: { title: 'AMC Plans', fields: ['name', 'fit', 'features'] },
  toolStack: { title: 'Tool Stack', fields: ['text'] },
  workingFlow: { title: 'Working Flow', fields: ['step', 'title', 'text'] },
  supportFaqs: { title: 'Support FAQs', fields: ['question', 'answer'] },
}
...
export default function TechnicalSupportManagement() {
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/technical-support-page')
      setPageData(data)
      setPageForm({
        ...initialPageForm,
      ...
      setError(requestError.message || 'Failed to load technical support page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/technical-support-page', { method: 'PUT', body: JSON.stringify(pageForm) })
      setMessage(result.message || 'Technical support page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      if ('features' in payload) payload.features = fromMultiline(payload.features)
      const editingId = editing[section]
      const path = editingId ? `/api/technical-support-page/${section}/${editingId}` : `/api/technical-support-page/${section}`
  ...
  const handleEditItem = (section, item) => {
    ...
    const nextValues = Object.fromEntries(
      config.fields.map((field) => [field, field === 'points' || field === 'features' ? toMultiline(item[field]) : item[field] || '']),
    )
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/technical-support-page/${section}/${itemId}`, { method: 'DELETE' })
  ...
  return (
    <DashboardShell
      ...
      subtitle="Ei dashboard theke TechnicalSupport page er shokol content (hero, coverage, matrix, plans, tools, workflow, videos, FAQ, CTA) CRUD korte parbe."
    >
              ...
              <form className="mt-3 space-y-2.5 rounded-xl border border-white/10 bg-white/[0.02] p-3" onSubmit={(event) => handleSectionSubmit(section, event)}>
                {sectionConfig[section].fields.map((field) => (
                  <div key={`${section}-${field}`}>
                    ...
                    {(field === 'description' || field === 'text' || field === 'points' || field === 'features' || field === 'answer' || field === 'example') ? (
                      <textarea
                        name={field}
                        rows={field === 'points' || field === 'features' ? 3 : 2}
                        placeholder={field === 'points' || field === 'features' ? 'One item per line' : ''}
              ...
              <ul className="mt-3 grid gap-2">
                {(pageData?.[section] || []).map((item) => (
                  <li key={item._id} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
                    <p className="text-sm text-white">{item.title || item.name || item.severity || item.step || item.question || item.text}</p>
                    {item.description || item.answer || item.example || item.fit ? <p className="text-xs text-white/70">{item.description || item.answer || item.example || item.fit}</p> : null}
                    {item.points?.length ? <p className="mt-1 text-xs text-white/60">{item.points.join(', ')}</p> : null}
                    {item.features?.length ? <p className="mt-1 text-xs text-white/60">{item.features.join(', ')}</p> : null}
