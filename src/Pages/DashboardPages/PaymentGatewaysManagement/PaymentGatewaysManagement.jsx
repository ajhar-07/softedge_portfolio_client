...
const API_PAGE = '/api/payment-gateways-page'
...
const initialForms = {
  ...
  rails: { title: '', text: '', gradient: '' },
  partnerChannels: { name: '', detail: '' },
  flowSteps: { phase: '01', title: '', text: '' },
  riskLayers: { id: 'R1', title: '', text: '' },
  walletCapabilities: { code: 'W1', heading: '', detail: '' },
  metrics: { value: '', label: '' },
  businessUseCases: { title: '', text: '' },
  faqs: { q: '', a: '' },
}
...
export default function PaymentGatewaysManagement() {
  ...
  const loadPageData = async () => {
    ...
    try {
      ...
      setError(requestError.message || 'Failed to load payment gateways page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    try {
      ...
      setMessage(result.message || 'Payment gateways page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleEditItem = (section, item) => {
    let nextForm
    if (section === 'heroStats' || section === 'metrics') nextForm = { value: item.value || '', label: item.label || '' }
    else if (section === 'serviceLinks') nextForm = { label: item.label || '', to: item.to || '' }
    else if (section === 'rails') nextForm = { title: item.title || '', text: item.text || '', gradient: item.gradient || '' }
    else if (section === 'partnerChannels') nextForm = { name: item.name || '', detail: item.detail || '' }
    else if (section === 'flowSteps') nextForm = { phase: item.phase || '', title: item.title || '', text: item.text || '' }
    else if (section === 'riskLayers') nextForm = { id: item.id || '', title: item.title || '', text: item.text || '' }
    else if (section === 'walletCapabilities') {
      nextForm = { code: item.code || '', heading: item.heading || '', detail: item.detail || '' }
    } else if (section === 'businessUseCases') nextForm = { title: item.title || '', text: item.text || '' }
  ...
  const renderSectionForm = (section) => {
    ...
    return (
      <form className="mt-5 space-y-4" onSubmit={(event) => handleSectionSubmit(section, event)}>
        {section === 'heroStats' || section === 'metrics' ? (
          <>
        ...
        {section === 'serviceLinks' ? (
          <>
            <input type="text" name="label" value={form.label} onChange={(event) => handleSectionFormChange(section, event)} placeholder="Label" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
            <input type="text" name="to" value={form.to} onChange={(event) => handleSectionFormChange(section, event)} placeholder="/payment-gateways" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
          </>
        ) : null}
        ...
        {section === 'rails' ? (
          <>
            ...
            <input type="text" name="gradient" value={form.gradient} onChange={(event) => handleSectionFormChange(section, event)} placeholder="from-[#00d2ff]/45 to-[#1b6ea1]/35" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
          </>
        ) : null}

        {section === 'partnerChannels' ? (
          <>
            <input type="text" name="name" value={form.name} onChange={(event) => handleSectionFormChange(section, event)} placeholder="Channel name" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
            <textarea name="detail" value={form.detail} onChange={(event) => handleSectionFormChange(section, event)} rows="3" placeholder="Channel detail" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
          </>
        ) : null}
        ...
        {section === 'riskLayers' ? (
          <>
            <input type="text" name="id" value={form.id} onChange={(event) => handleSectionFormChange(section, event)} placeholder="R1" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
            <input type="text" name="title" value={form.title} onChange={(event) => handleSectionFormChange(section, event)} placeholder="Layer title" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
            <textarea name="text" value={form.text} onChange={(event) => handleSectionFormChange(section, event)} rows="3" placeholder="Layer text" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
          </>
        ) : null}

        {section === 'walletCapabilities' ? (
          <>
            <input type="text" name="code" value={form.code} onChange={(event) => handleSectionFormChange(section, event)} placeholder="W1" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
        ...
        {section === 'businessUseCases' ? (
          <>
            <input type="text" name="title" value={form.title} onChange={(event) => handleSectionFormChange(section, event)} placeholder="Use case title" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
            <textarea name="text" value={form.text} onChange={(event) => handleSectionFormChange(section, event)} rows="3" placeholder="Use case text" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40" required />
          </>
        ) : null}
  ...
  const renderItemTitle = (section, item) => {
    if (section === 'heroStats' || section === 'metrics' || section === 'serviceLinks') return item.label
    if (section === 'partnerChannels') return item.name
    if (section === 'riskLayers') return `${item.id || ''} — ${item.title || ''}`
    if (section === 'walletCapabilities') return `${item.code || ''} — ${item.heading || ''}`
    if (section === 'faqs') return item.q
    return item.title
  }

  const renderItemDescription = (section, item) => {
    if (section === 'heroStats' || section === 'metrics') return item.value
    if (section === 'serviceLinks') return item.to
    if (section === 'rails') return `${item.text || ''} (${item.gradient || ''})`
    if (section === 'partnerChannels') return item.detail
    if (section === 'flowSteps') return `${item.phase || ''} — ${item.text || ''}`
    if (section === 'riskLayers') return item.text
    if (section === 'walletCapabilities') return item.detail
    if (section === 'businessUseCases') return item.text
    return item.a
  }

  return (
    <DashboardShell
      title="Payment gateways management"
      subtitle={
        <>
          Public page:{' '}
          <Link to="/payment-gateways" className="text-[#00d2ff] underline hover:text-white">
            /payment-gateways
          </Link>{' '}
          — hero, stats, rails, channels, flow, risk, wallet, metrics, use cases and FAQs — CRUD from here.
        </>
      }
    >
          ...
          <div className="rounded-[28px] border border-white/10 bg-[#000b1e]/42 p-5 shadow-[0_12px_35px_-15px_rgba(0,0,0,0.52)] sm:p-6">
            ...
            <form className="mt-6 space-y-4" onSubmit={handlePageSubmit}>
              ...
              <label className="block">
                ...
                <textarea name="heroLead" value={pageForm.heroLead} onChange={handlePageChange} rows="4" className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none transition focus:border-[#00d2ff]/40" />
              </label>
            ...
            {loading ? (
              ...
              <>
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-xs text-white/60">
                  ...
                  <p className="mt-1 break-words"><span className="text-[#00d2ff]">heroBadge:</span> {pageData.heroBadge}</p>
                </div>
                {DASHBOARD_SECTIONS.map((section) => (
