...
export default function ContentManagement() {
  ...
  const loadPageData = async () => {
    ...
    setError('')
    try {
      const data = await apiRequest('/api/content-management-page')
      setPageData(data)
      setPageForm({
        heroImage: data.heroImage || '',
        mainImage: data.mainImage || '',
        heroTitle: data.heroTitle || 'Content Management',
        sectionTitle: data.sectionTitle || 'Content Management',
        sectionDescription: data.sectionDescription || '',
        sectionSecondaryDescription: data.sectionSecondaryDescription || '',
      ...
      setError(requestError.message || 'Failed to load content management page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleGalleryFileUpload = async (event) => {
    ...
    try {
      ...
      setSectionForms((current) => ({
        ...current,
        gallery: { ...current.gallery, url: uploadedUrl },
      }))
  ...
  const handlePageSubmit = async (event) => {
    ...
    setError('')
    try {
      const result = await apiRequest('/api/content-management-page', {
        method: 'PUT',
        body: JSON.stringify(pageForm),
      })
      setMessage(result.message || 'Content management page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      const path = editingId
        ? `/api/content-management-page/${section}/${editingId}`
        : `/api/content-management-page/${section}`
  ...
  const handleDeleteItem = async (section, itemId) => {
    ...
    setError('')
    try {
      const result = await apiRequest(`/api/content-management-page/${section}/${itemId}`, {
        method: 'DELETE',
      })
    ...
    return (
      <form className="mt-5 space-y-4" onSubmit={(event) => handleSectionSubmit(section, event)}>
        ...
        {section === 'gallery' ? (
          <>
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-white/82">Or upload image</span>
              <input
                type="file"
                accept="image/*"
              ...
              <p className="mt-2 text-xs text-white/50">
                {uploadingField === 'gallery-url' ? 'Uploading...' : 'Upload sets the URL field above'}
              </p>
            </label>
          </>
        ) : null}
        ...
        <div className="flex flex-wrap gap-3">
          <button
            type="submit"
            disabled={savingSection === section || (section === 'gallery' && uploadingField === 'gallery-url')}
  ...
  return (
    <DashboardShell
      ...
      subtitle="Ei dashboard theke content management public page er shob content create, read, update, delete korte parben."
    >
      <div className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="rounded-[28px] border border-white/10 bg-[#000b1e]/42 p-5 shadow-[0_12px_35px_-15px_rgba(0,0,0,0.52)] sm:p-6">
            ...
            <form className="mt-6 space-y-4" onSubmit={handlePageSubmit}>
              {[
                ...
                ['sectionDescription', 'Section description (first paragraph)'],
                ['sectionSecondaryDescription', 'Section description (second paragraph)'],
              ...
              <div className="space-y-4">
                {[
                  ...
                  ['mainImage', 'Main content image upload'],
                ].map(([field, label]) => (
        ...
        <div className="rounded-[28px] border border-white/10 bg-[#000b1e]/42 p-5 shadow-[0_12px_35px_-15px_rgba(0,0,0,0.52)] sm:p-6">
          ...
          <h2 className="mt-3 text-2xl font-black text-white">Live content management data</h2>
                          ...
                          <div key={item._id} className="rounded-2xl border border-white/10 bg-[#0a3146]/22 p-4">
                            <div className="flex items-start justify-between gap-4">
                              <div className="min-w-0 flex-1">
                                ...
                                <p className="mt-2 break-all text-sm leading-7 text-white/68">
                                  {renderItemDescription(section, item)}
                                </p>
                                {section === 'gallery' && item.url ? (
                                  <img
                                    src={item.url}
                                    alt=""
                                    className="mt-3 h-20 w-full max-w-xs border border-white/10 object-cover"
                                  />
                                ) : null}
                              </div>
