...
const API_PAGE = '/api/municipality-union-management-page'
...
const initialForms = {
  ...
  civicModules: { title: '', text: '', icon: '' },
  processTimeline: { step: 'Phase 01', title: '', text: '' },
  performanceCards: { label: '', value: '' },
...
const sectionLabels = {
  ...
  civicModules: 'Core civic modules',
  processTimeline: 'Service processing journey',
  performanceCards: 'Performance snapshot cards',
  governanceFeatures: 'Governance feature bullets',
  faq: 'FAQs',
}
...
export default function MinicipalityManagement() {
  ...
  const loadPageData = async () => {
    ...
    try {
      ...
      setError(requestError.message || 'Failed to load municipality management page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    try {
      ...
      setMessage(result.message || 'Municipality management page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleEditItem = (section, item) => {
    ...
    else if (section === 'civicModules') {
      nextForm = { title: item.title || '', text: item.text || '', icon: item.icon || '' }
    } else if (section === 'processTimeline') {
      nextForm = { step: item.step || 'Phase 01', title: item.title || '', text: item.text || '' }
    } else if (section === 'performanceCards') {
      nextForm = { label: item.label || '', value: item.value || '' }
    } else if (section === 'governanceFeatures') {
      nextForm = { text: item.text || '' }
    } else {
      nextForm = { q: item.q || '', a: item.a || '' }
    }
        ...
        {section === 'serviceLinks' ? (
          <>
            ...
            <input
              ...
              value={form.to}
              onChange={(event) => handleSectionFormChange(section, event)}
              placeholder="/municipality-union-management"
              className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none focus:border-[#00d2ff]/40"
              required
            />
          </>
  ...
  const renderItemTitle = (section, item) => {
    if (section === 'serviceLinks' || section === 'performanceCards') return item.label
    if (section === 'civicModules' || section === 'processTimeline') return item.title
    if (section === 'governanceFeatures') return 'Feature item'
    return item.q
  }

  const renderItemDescription = (section, item) => {
    if (section === 'serviceLinks') return item.to
    if (section === 'civicModules') return `${item.icon || ''} — ${item.text || ''}`
    if (section === 'processTimeline') return `${item.step || ''} — ${item.text || ''}`
    if (section === 'performanceCards') return item.value
    if (section === 'governanceFeatures') return item.text
    return item.a
  }

  return (
    <DashboardShell
      title="Municipality / Union Management"
      subtitle={
        <>
          Public page:{' '}
          <Link to="/municipality-union-management" className="text-[#00d2ff] underline hover:text-white">
            /municipality-union-management
          </Link>{' '}
          — hero, civic modules, processing timeline, KPI, governance features, FAQ — CRUD from here.
        </>
      }
    >
              ...
              <>
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-xs text-white/60">
                  ...
                  <p className="mt-1 break-words">
                    <span className="text-[#00d2ff]">heroBadge:</span> {pageData.heroBadge}
                  </p>
                </div>
                {DASHBOARD_SECTIONS.map((section) => (
