...
const API_PAGE = '/api/security-audit-page'
...
const initialForms = {
  navPills: { label: '', to: '' },
  stats: { label: '', value: '' },
  auditVsPentest: { tag: '', title: '', body: '', bulletsInput: '' },
  methodology: { phase: '', detail: '' },
  coverage: { area: '', note: '' },
...
const sectionLabels = {
  ...
  auditVsPentest: 'Audit vs pentest blocks',
  methodology: 'Methodology (timeline)',
  coverage: 'Coverage areas',
  deliverables: 'Deliverable cards',
  frameworks: 'Framework chips',
  faqs: 'FAQs',
}
...
export default function SecurityAuditManagement() {
  ...
  const loadPageData = async () => {
    ...
    try {
      ...
      setPageForm({
        ...
        methodologySectionEyebrow: data.methodologySectionEyebrow || '',
        methodologySectionTitle: data.methodologySectionTitle || '',
        methodologySectionSubtitle: data.methodologySectionSubtitle || '',
        coverageSectionEyebrow: data.coverageSectionEyebrow || '',
        coverageSectionTitle: data.coverageSectionTitle || '',
        deliverablesSectionEyebrow: data.deliverablesSectionEyebrow || '',
        deliverablesSectionTitle: data.deliverablesSectionTitle || '',
        frameworksSectionEyebrow: data.frameworksSectionEyebrow || '',
        frameworksSectionTitle: data.frameworksSectionTitle || '',
        frameworksSectionSubtitle: data.frameworksSectionSubtitle || '',
      ...
      setError(requestError.message || 'Failed to load security audit page data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageSubmit = async (event) => {
    ...
    try {
      ...
      setMessage(result.message || 'Security audit page updated successfully')
      await loadPageData()
    } catch (requestError) {
  ...
  const handleSectionSubmit = async (section, event) => {
    ...
    try {
      ...
      let payload = form
      if (section === 'auditVsPentest') {
        payload = {
          tag: form.tag,
          title: form.title,
          body: form.body,
          bullets: form.bulletsInput
            .split('\n')
            .map((s) => s.trim())
            .filter(Boolean),
        }
  ...
  const handleEditItem = (section, item) => {
    ...
    else if (section === 'auditVsPentest')
      nextForm = {
        tag: item.tag || '',
        title: item.title || '',
        body: item.body || '',
        bulletsInput: Array.isArray(item.bullets) ? item.bullets.join('\n') : '',
      }
    else if (section === 'methodology') nextForm = { phase: item.phase || '', detail: item.detail || '' }
    else if (section === 'coverage') nextForm = { area: item.area || '', note: item.note || '' }
    else if (section === 'deliverables') nextForm = { title: item.title || '', desc: item.desc || '' }
    else if (section === 'frameworks') nextForm = { text: item.text || '' }
      ...
      <form className="mt-5 space-y-4" onSubmit={(event) => handleSectionSubmit(section, event)}>
        {section === 'navPills' ? (
          <>
            ...
            <input type="text" name="to" value={form.to} onChange={(e) => handleSectionFormChange(section, e)} placeholder="/security-audit" className={inputClass} required />
          </>
        ) : null}
        ...
        {section === 'auditVsPentest' ? (
          <>
            <input type="text" name="tag" value={form.tag} onChange={(e) => handleSectionFormChange(section, e)} placeholder="Tag (e.g. ASSESS)" className={inputClass} required />
            ...
            <textarea name="body" value={form.body} onChange={(e) => handleSectionFormChange(section, e)} rows="4" placeholder="Body" className={textareaClass} required />
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-white/82">Bullets (one per line)</span>
              <textarea name="bulletsInput" value={form.bulletsInput} onChange={(e) => handleSectionFormChange(section, e)} rows="6" className={textareaClass} required={!editingId} />
            </label>
          </>
        ) : null}
        ...
        {section === 'methodology' ? (
          <>
            <input type="text" name="phase" value={form.phase} onChange={(e) => handleSectionFormChange(section, e)} placeholder="Phase (e.g. 01 — Scoping)" className={inputClass} required />
        ...
        {section === 'coverage' ? (
          <>
            <input type="text" name="area" value={form.area} onChange={(e) => handleSectionFormChange(section, e)} placeholder="Area" className={inputClass} required />
            <textarea name="note" value={form.note} onChange={(e) => handleSectionFormChange(section, e)} rows="3" placeholder="Note" className={textareaClass} required />
          </>
        ) : null}
        ...
        {section === 'deliverables' ? (
          <>
            <input type="text" name="title" value={form.title} onChange={(e) => handleSectionFormChange(section, e)} placeholder="Title" className={inputClass} required />
            <textarea name="desc" value={form.desc} onChange={(e) => handleSectionFormChange(section, e)} rows="4" placeholder="Description" className={textareaClass} required />
          </>
        ) : null}

        {section === 'frameworks' ? (
          <input type="text" name="text" value={form.text} onChange={(e) => handleSectionFormChange(section, e)} placeholder="Framework name" className={inputClass} required />
        ) : null}
  ...
  const renderItemTitle = (section, item) => {
    ...
    if (section === 'auditVsPentest') return item.title || item.tag
    if (section === 'methodology') return item.phase
    if (section === 'coverage') return item.area
    if (section === 'deliverables') return item.title
    if (section === 'frameworks') return item.text
    return item.q
  }
  ...
  const renderItemDescription = (section, item) => {
    ...
    if (section === 'auditVsPentest') return [item.body, ...(item.bullets || []).slice(0, 2).map((b) => `• ${b}`)].filter(Boolean).join('\n')
    if (section === 'methodology') return item.detail
    if (section === 'coverage') return item.note
    if (section === 'deliverables') return item.desc
    if (section === 'frameworks') return item.text
    return item.a
  }
  ...
  const scalarTextareas = [
    ...
    ['methodologySectionSubtitle', 'Methodology section — subtitle'],
    ['frameworksSectionSubtitle', 'Frameworks section — subtitle'],
    ['ctaSubtitle', 'CTA strip — subtitle'],
  ]

  return (
    <DashboardShell
      title="Security audit + pentest management"
      subtitle={
        <>
          Public page:{' '}
          <Link to="/security-audit" className="text-[#00d2ff] underline hover:text-white">
            /security-audit
          </Link>{' '}
          — hero, audit blocks, methodology, coverage, FAQs — CRUD from here.
        </>
      }
    >
          ...
          <div className="rounded-[28px] border border-white/10 bg-[#000b1e]/42 p-5 shadow-[0_12px_35px_-15px_rgba(0,0,0,0.52)] sm:p-6">
            ...
            <form className="mt-6 max-h-[70vh] space-y-3 overflow-y-auto pr-1" onSubmit={handlePageSubmit}>
              {[
                ...
                ['title', 'Hero title'],
                ['methodologySectionEyebrow', 'Methodology — eyebrow'],
                ['methodologySectionTitle', 'Methodology — title'],
                ['coverageSectionEyebrow', 'Coverage — eyebrow'],
                ['coverageSectionTitle', 'Coverage — title'],
                ['deliverablesSectionEyebrow', 'Deliverables — eyebrow'],
                ['deliverablesSectionTitle', 'Deliverables — title'],
                ['frameworksSectionEyebrow', 'Frameworks — eyebrow'],
                ['frameworksSectionTitle', 'Frameworks — title'],
                ['faqSectionEyebrow', 'FAQ — eyebrow'],
                ['faqSectionTitle', 'FAQ — title'],
                ['ctaTitle', 'CTA — title'],
                ['ctaPrimaryLabel', 'CTA — primary button label'],
                ['ctaPrimaryTo', 'CTA — primary path'],
                ['ctaSecondaryLabel', 'CTA — secondary button label'],
